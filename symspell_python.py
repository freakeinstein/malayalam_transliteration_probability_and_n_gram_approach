...
def create_dictionary_entry(w):
    ...
    if w in dictionary:
        #print("{} exists already in dictionary".format(w))
        # increment count of word in corpus
        dictionary[w] = (dictionary[w][0], dictionary[w][1] + 1)  
    else:
        #print("{} is new in dictionary".format(w))
        dictionary[w] = ([], 1)  
...
def add_to_db(malayalam):
    db=MySQLdb.connect(user="root",passwd="scott",db="amma",charset='utf8',use_unicode=True)
    c=db.cursor()

def create_dictionary(speed=100,segment=1000):
    print("db fetch..",flush="true")
    ...
    c.execute("SELECT COUNT(*) FROM `amma`.`words_corpus` where `count` > "+str(speed)+" ORDER BY `count` DESC")
    lnn = c.fetchone()
    no_segments = int(lnn[0]/segment)
    no_remain = lnn[0]%segment

    #sxxx = segment%1000
    
    print("Total words to be processed: {}".format(lnn))

    seg_count = 0
    while(seg_count <= no_segments):
        c.execute("SELECT `word`, `count` FROM `amma`.`words_corpus` where `count` > "+str(speed)+" ORDER BY `count` DESC LIMIT "+str(segment*seg_count)+","+str(segment))
        #print(str(segment*seg_count)+","+str(segment))
        #print(c.rowcount,end='')

        cnt=1
        fetch = c.fetchone()
        while(fetch):
            if(cnt >= 100):
                print("processed: {} words, completed: {} % of total words to process \r".format(total_word_count+1,round(total_word_count/lnn[0]*100)),flush=True,end="")
                cnt = 1
            cnt += 1
            total_word_count += 1
            if create_dictionary_entry(fetch[0]):
                unique_word_count += 1
            fetch = c.fetchone() 
        seg_count += 1
...
if __name__ == "__main__":
    ...
    try:
        create_dictionary("malayalam.txt")
    except:
